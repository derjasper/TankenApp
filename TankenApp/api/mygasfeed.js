...

{
    function processLastUpdate(str) {
        ...
        if (diff != null)
            lastUpdate = new Date((new Date()).getTime() - diff).toISOString();
        ...
        function processList(jsonString) {
            ...
            for ( var key in data ) {
                model.push({
                    ...
                    price: processPrice(data[key][type+"_price"]),
    ...
    mygasfeedApi.getDetails = function(apikey, url, id, callback) {
        ...
        function processEntry(jsonString) {
            ...
            if (data.reg_price != null) {
                obj.fuel.push({
                    type: "reg",
                    price: processPrice(data.reg_price),
            ...
            if (data.mid_price != null) {
                obj.fuel.push({
                    type: "mid",
                    price: processPrice(data.mid_price),
                    price_currency: (data.country == "Canada" ? "CAD": "USD"),
            ...
            if (data.pre_price != null) {
                obj.fuel.push({
                    type: "pre",
                    price: processPrice(data.pre_price),
                    price_currency: (data.country == "Canada" ? "CAD": "USD"),
            ...
            if (data.diesel_price != null) {
                obj.fuel.push({
                    type: "diesel",
                    price: processPrice(data.diesel_price),
                    price_currency: (data.country == "Canada" ? "CAD": "USD"),
